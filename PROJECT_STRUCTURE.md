...
- **Key Types:** `PlayoutAdapter` (pluggable audio backend, optional `addTrack()` for incremental additions), `EngineState` (state snapshot incl. `tracksVersion`/`mixerVersion`), `EngineEvents` (statechange, play/pause/stop)
