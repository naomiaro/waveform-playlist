...
- **The community registry mixes WAM 1.0 entries** — e.g. wimmics/BigMuff fails `createWamInstance` with the apiVersion error. Known-good WAM 2.0 effects for demos: `burns-audio/delay` ("Simple Delay"), `burns-audio/distortion`. **Registry-reality (2026-07-02 live survey, all 58 community entries):** 33 declare `apiVersion: "1.x"` in their static `descriptor.json` (all Wimmics/legacy) — a static probe can *positively* identify these. The other 25 OMIT `apiVersion` entirely (WAM 2.0 SDK builds) — the SDK only stamps `apiVersion` on the RUNTIME instance descriptor, never reliably on the static file. The picker rule everywhere is therefore `apiVersion?.startsWith('1')` → legacy/not insertable, and **absent → insertable** (never require the field, never treat absence as 1.0). `createWamInstance` stays authoritative at load time regardless of what the static probe says.
- **burns-audio plugin quirks (verified by measurement):** simpleDistortion's defaults are near-transparent (−6 dB pad, no added harmonics — `overdrive` defaults to its declared MINIMUM of 1); crank `overdrive` toward 10 for audible demos. Their bundled GUIs can send values past declared param ranges (e.g. `level` 2.22 vs [0,2]) — the browser clamps with console warnings; cosmetic plugin-side bug, not a host issue. Host output verified byte-equivalent between dawcore and React for identical plugin/params (effected/bypassed RMS ratios within 1%).
- **`defaultImport` carries `/* webpackIgnore: true */ /* @vite-ignore */`** — both directives are load-bearing (see root CLAUDE.md pattern #14); don't strip either when touching the loader.
...
- `fetchWamDescriptor(pluginUrl, { fetchFn? })` → `WamDescriptorInfo | null` — best-effort probe of the static `descriptor.json` SDK builds ship next to the module (`.../index.js` → `.../descriptor.json`); audio-I/O truth where registry `category` lies (e.g. "Audio Input" is tagged Effect but has `hasAudioInput=false`). Null on ANY failure by contract (no throw, no warning) — absence is expected for non-SDK registries; callers fall back to `category`, and `createWamInstance` stays authoritative. **Absent flags ≠ false**: static descriptors may omit flags (2 of 58 community entries do) and the SDK runtime-defaults audio I/O to `true` — gate on `!== false`, never `=== true`. A flag-less descriptor returns `{}`, distinct from `null`. **`apiVersion` (#539)**: also passed through when the payload has it as a non-empty string. A static probe can only *positively* identify WAM 1.0 (declared `"1.x"`) — the SDK stamps `apiVersion` reliably only on the runtime instance descriptor, so an absent field on the static file means "unknown", typically a 2.0 SDK build. Picker rule: `apiVersion?.startsWith('1')` → legacy/not insertable; absent → insertable.
