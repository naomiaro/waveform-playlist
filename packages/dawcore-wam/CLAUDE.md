...
- **The community registry mixes WAM 1.0 entries** — e.g. wimmics/BigMuff fails `createWamInstance` with the apiVersion error. Known-good WAM 2.0 effects for demos: `burns-audio/delay` ("Simple Delay"), `burns-audio/distortion`. #539 tracks exposing `apiVersion` from `fetchWamDescriptor` for picker filtering.
