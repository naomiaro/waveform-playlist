...
- **Common-denominator schema** (surveyed from webaudiomodules.com community `plugins.json` and pedalboard2/wam-studio libraries): top-level array OR `{ plugins: [...] }`; entries are objects (`name` + `url`/`path` required; optional description/vendor/thumbnail/keywords/category passed through when well-formed; `category` normalized to a string array (bare string wrapped, non-strings filtered, empty-after-filter omitted) — the community registry categorizes every entry (Effect/Modulation/Instrument/MIDI/Video) and consumers use it to gate effects-chain insertability) or bare URL strings (name derived from URL, skipping generic segments like `index.js`/`dist`/`src`).
