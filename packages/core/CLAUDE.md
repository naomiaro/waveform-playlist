...
| `enumerateMicrophones.ts` | `enumerateMicrophones(mediaDevices?)` + `watchMicrophoneDevices(listener, mediaDevices?)` — permission-free audio-input listing with `supported` (SSR/insecure-context safe) and `hasLabels` (false pre-permission: browsers redact labels and often deviceIds; fallback names generated) flags. Injectable `mediaDevices` for tests. `useMicrophoneAccess` (recording) delegates to it |
| `constants.ts` | `MAX_CANVAS_WIDTH` (1000px virtual scroll chunk size) |
| `spectrogramCanvasId.ts` | Canonical spectrogram canvas-ID contract: `buildSpectrogramCanvasId`/`parseSpectrogramCanvasId` for the `${clipId}-ch${channelIndex}-chunk${n}` format. Single source consumed by the worker pool, React helpers, `SpectrogramChannel`, and `<daw-spectrogram>` — change the format here only (#560) |
