...
**Web Audio Automation Ordering:** `setValueAtTime` at note-off correctly cancels incomplete `linearRampToValueAtTime` ramps — do NOT use `Math.max(noteOff, envEnd)` to "fix" ordering. That extends every note to the full AHD phase length, breaking instruments with long decay (piano, strings). The original `time + duration` approach is correct for notes that outlive the decay. **CONFIRMED bug for staccato notes (Chromium OfflineAudioContext gain-curve capture, 2026-07-09):** when note-off lands mid-decay, `setValueAtTime` does NOT resume from the ramp's current value — the gain holds at PEAK for the whole note (the decay ramp's endpoint sorts after note-off, so it never governs), then steps peak→sustain in ONE sample at note-off (measured 0.576 jump at velocity 0.8, sustain 0.1 — an audible click). The orphaned decay ramp also re-raises the rendered gain after release, but `source.stop()` makes that inaudible. Verified fix: `cancelAndHoldAtTime(noteOff)` + release ramp — max sample step 0 (needs a Firefox fallback computing the held value analytically; `cancelAndHoldAtTime` support there is recent).
