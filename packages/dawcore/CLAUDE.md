...
- **`RecordingController.stopRecording` returns `Promise<void>`** — awaits the worklet's `done: true` ack before reading chunks (same handshake as `useRecording`). Fire-and-forget callers (`daw-stop-button`) work unchanged. Test mocks must auto-acknowledge `{ command: 'stop' }` by triggering `onmessage` with `{ done: true, channels: [], channelCount }`.
