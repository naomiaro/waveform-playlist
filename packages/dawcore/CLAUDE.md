...
- **Stream channelCount unreliable for stereo mics** — `getSettings().channelCount` returns `undefined` on some browsers for stereo devices. `RecordingOptions.channelCount` provides a fallback (matches React `useRecording` pattern). Default is `1` if neither detection nor option provides a value.
