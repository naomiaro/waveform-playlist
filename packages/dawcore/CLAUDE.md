...
- **`audioContext` JS Property** — Set `editor.audioContext = new AudioContext({ sampleRate: 48000, latencyHint: 0 })` before tracks load. Passed to `configureGlobalContext({ audioContext })` in `_ensureContextConfigured`. Takes precedence over `sample-rate` attribute. All dev pages set this.
