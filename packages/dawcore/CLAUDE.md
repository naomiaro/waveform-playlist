...
- **`src/effects/`** — `EffectsChainController` (ordered chain between owned input/output gains) + effect registry (`registerEffect`/`getEffectDefinitions`/`createEffectInstance`, five `native-*` built-ins) + `EffectsManager` (per-editor chain ownership/wiring/events). WAM entries join via #422.
