...
- **`<daw-grid>` element** — Shadow DOM, chunked 1000px canvases (same pattern as `<daw-waveform>`). Positioned behind tracks via `z-index: 0` inside the `.scroll-area`. Track rows go transparent via `:host([scale-mode="beats"]) .track-row { background: transparent }`. Grid naturally inherits the scroll position from the timeline.
