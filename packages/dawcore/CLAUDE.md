...
- **Pre-Computed Peaks Skip Worker** — When `peaks-src` loads a `.dat` file, the WaveformData is cached in `PeakPipeline` via `cacheWaveformData(audioBuffer, waveformData)`. All downstream paths (zoom re-extraction, split/trim sync, trim visual feedback) use the cached data — the worker is never invoked. If the `.dat` scale is coarser than the requested zoom, `_clampScale` warns and returns peaks at the available resolution.
