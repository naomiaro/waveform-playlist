...
- **`editor.bpm` is display-only when tick callbacks are set** — with both `secondsToTicks`/`ticksToSeconds` provided, the setter and `_buildEngine` skip engine/adapter tempo forwarding entirely (the external tempo map is authoritative — #407). Without callbacks, `editor.bpm` forwards to `engine.setTempo` → `adapter.setTempo(bpm, tick 0)` as before. Caveat: the engine's internal fallback bpm freezes at its build-time value in callbacks mode — only relevant for a custom adapter that lacks `ticksToSeconds`/`secondsToTicks` (NativePlayoutAdapter implements both). Diagnostic for off-grid beats: constant per-beat offset = pre-roll tempo wrong (check `transport.getTempo(0)`); growing offset = per-beat entries wrong.
