...
- **`tracksVersion` counter** — Monotonic counter in `EngineState` that increments on track mutations (setTracks, addTrack, removeTrack, moveClip, trimClip, splitClip) AND per-track mixer changes (setTrackVolume, setTrackMute, setTrackSolo, setTrackPan — #501). Does NOT increment on selection/zoom/**master** volume/loop changes. Used by consumers to detect track-state statechange events. The React provider additionally relies on the `isEngineTracks` reference guard to skip `loadAudio` rebuilds when it mirrors these back via `onTracksChange` — so per-track mixer changes now also reach the provider's `onTracksChange` (previously suppressed).
...
- **Track audio state persistence** — `setTrackVolume/setTrackMute/setTrackSolo/setTrackPan` update `this._tracks[]` (so a later adapter rebuild keeps the value), forward to the adapter, AND bump `tracksVersion` + emit `statechange` like every other track-mutating method (#501). The adapter forward stays unconditional even when the track is absent from `_tracks` (a consumer may target a track the engine hasn't ingested yet).
