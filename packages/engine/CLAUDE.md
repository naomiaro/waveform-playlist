...
**Key types:** `PlayoutAdapter` (pluggable audio backend interface — requires `readonly audioContext: AudioContext`, optional `readonly ppqn?: number`), `EngineState` (state snapshot), `EngineEvents` (statechange, play/pause/stop).
