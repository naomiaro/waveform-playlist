...
- **Unconditional loop activation** — `play()`, `setLoopEnabled()`, and `setLoopRegion()` always pass `isLoopEnabled` directly to `adapter.setLoop()` without position checks. Tone.js Transport handles loop boundary wrapping natively — if playback starts before or after the loop region, Transport still wraps correctly at `loopEnd`. Selection/annotation playback (with `endTime`) always disables loop.
