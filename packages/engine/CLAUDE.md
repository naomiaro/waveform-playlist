...
- `PlayoutAdapter.isPlaying()` is defined but not called by engine (engine tracks own `_isPlaying`) — bounded completion is now observed via `onPlaybackEnded` (#608); `isPlaying()` polling remains unused by design.
- **`onPlaybackEnded` hook (#608)** — optional `PlayoutAdapter.onPlaybackEnded(callback | null)`, for adapters that self-terminate a duration-limited `play(start, end)` (annotation/selection playback). Constructor subscribes once: `this._adapter?.onPlaybackEnded?.(() => queueMicrotask(() => { if (this._isPlaying && !this._disposed) this.stop(); }))`. The `queueMicrotask` defer matters because the Tone adapter fires this from inside the Transport tick chain, which catches nothing (see playout CLAUDE.md) — stopping the Transport re-entrantly from a tick callback is the hazard the microtask sidesteps. Re-checking `_isPlaying` AND `_disposed` inside the microtask covers a consumer `stop()`/`dispose()` landing in the gap between the adapter's callback firing and the microtask running.
- **Single callback slot, last-subscriber-wins** — `onPlaybackEnded` takes exactly one callback, not a listener set — matches the repo-wide one-engine-per-adapter convention (no fan-out, no unsubscribe-by-reference needed).
- **Dispose unsubscribes before tearing down the adapter** — `dispose()` sets `_disposed = true`, then `_isPlaying = false`, then calls `this._adapter?.onPlaybackEnded?.(null)` BEFORE `this._adapter?.dispose()` (itself wrapped in try/catch). This guarantees a disposed engine can't be re-entered by a late adapter callback; the microtask's own `_disposed` check is the second layer of defense for a callback that was already in flight when dispose ran.
...
- **`tracksVersion` counter** — Monotonic counter in `EngineState` that increments only on **structural** track mutations (setTracks, addTrack, removeTrack, moveClip, trimClip, splitClip). Does NOT increment on selection/zoom/master volume/loop changes, NOR on per-track mixer edits (those bump `mixerVersion` — see below). Used by the provider to detect structural statechange events and skip `loadAudio` rebuilds. Keeping mixer edits OUT of `tracksVersion` is deliberate: consumers gate expensive structural work (audio-graph rewire, peak regeneration) on it, which must not run on every volume/pan drag frame.
- **`mixerVersion` counter (#501)** — Monotonic counter in `EngineState` that increments on per-track mixer edits (setTrackVolume, setTrackMute, setTrackSolo, setTrackPan). Lets a consumer that caches a track snapshot (e.g. `@dawcore/components`' `<daw-editor>` `_engineTracks`) refresh that cache so live mixer state survives a later `setTracks` rebuild — WITHOUT triggering the `tracksVersion`-gated structural work. The React provider doesn't need it (it persists mixer state via `trackStates`).
...
- **Track audio state persistence** — `setTrackVolume/setTrackMute/setTrackSolo/setTrackPan` update `this._tracks[]` (so a later adapter rebuild keeps the value), forward to the adapter, AND bump `mixerVersion` + emit `statechange` (#501) — NOT `tracksVersion` (these are not structural changes). The adapter forward stays unconditional even when the track is absent from `_tracks` (a consumer may target a track the engine hasn't ingested yet).
