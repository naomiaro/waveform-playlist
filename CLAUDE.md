...
34. **SoundTouch AudioWorklet + SAC Compatibility** — `SoundTouchNode` extends native `AudioWorkletNode` which rejects SAC contexts. Skip the class: register via `rawContext.audioWorklet.addModule(url)`, create via `context.createAudioWorkletNode(name, options)`. For connecting SAC worklet nodes to Tone.js nodes, use Tone.js `Gain` bridges (same as pattern #32). For MediaElement + SoundTouch: set `preservesPitch={false}`, drive speed via `audioElement.playbackRate`, set `stNode.playbackRate` param (not `tempo`) for pitch compensation — `tempo` param does internal time-stretching in 128-sample blocks which sounds bad.
