...
30. **Vite Externals Derived from peerDependencies** — `packages/browser/vite.config.ts` reads `peerDependencies` from `package.json` and externalizes them automatically (including deep imports like `@dnd-kit/abstract/modifiers`). Never add a manual external list — it drifts when dependencies change, causing duplicate instances at runtime (#317).
