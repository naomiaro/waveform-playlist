...
48. **Pre-Computed Peaks Require Sample Rate Match** — `.dat` file `sample_rate` must match `AudioContext.sampleRate`. On mismatch, `createClip` warns per-clip and callers fall back to worker-generated peaks. Both dawcore (rejects `.dat`, falls to worker) and browser (converts offsets for preview, worker replaces) handle this. `configureGlobalContext({ sampleRate })` from playout requests a matching rate but hardware may not comply.
