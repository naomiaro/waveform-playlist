...
- **TypeScript check**: `pnpm typecheck` (enforced in build scripts). TS strict mode rejects unused private methods with `TS6133` — when extracting a helper, wire it into all call sites in the same change. Staged "add helper now, use it later" commits will fail typecheck.
