...
49. **Pass Your Own AudioContext for Sample Rate Control** — `WaveformPlaylistProvider` accepts an `audioContext` prop; `<daw-editor>` accepts an `audioContext` JS property. Create `new AudioContext({ sampleRate: 48000, latencyHint: 0 })` and pass it in. This bypasses Tone.js 15.1.22's limitation (doesn't wire `sampleRate` through). `configureGlobalContext({ audioContext })` wraps it in `new Context(rawContext)`. All website examples use a shared singleton from `website/src/components/audioContext.ts`.
