...
56. **Cross-Context Worklet Support (limiter-audio-worklet Pattern)** — `new AudioWorkletNode(ctx, ...)` fails when `ctx` is standardized-audio-context (Tone.js). Use callback injection: `addRecordingWorkletModule((url) => ctx.audioWorklet.addModule(url))` from worklets package. For node creation, `PlayoutAdapter` exposes optional `createAudioWorkletNode`/`createMediaStreamSource` methods — adapters implement these using their context type. `<daw-editor>` bridges via `RecordingHost` interface methods, falling back to native APIs.
