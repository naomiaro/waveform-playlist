...
- Renders one **annotation lane** per registered annotation track: a fixed-height row (32px default, themeable via `--daw-annotation-lane-height`) rendered above the audio track rows, inside `.timeline` — so horizontal scroll, zoom (`samplesPerPixel` / beats-mode tick math), and the playhead crossing come for free from the frozen-panes layout.
