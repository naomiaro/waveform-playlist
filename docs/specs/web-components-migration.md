...
3. **SSR** — Open. Lit supports Declarative Shadow DOM for server rendering. Full editor requires AudioContext (client-only), but a read-only subset (static waveforms from pre-generated peaks, track layout, timescale) could SSR/SSG meaningfully. Worth exploring if users want to render waveform previews server-side without JavaScript.
4. **Backwards compatibility period** — Decided: develop on a long-lived feature branch (e.g., `feat/dawcore`). Each migration phase gets its own branch off the feature branch, merged back when complete. Merge to `main` only when fully ready for release. Existing v10 users stay on `main` until then.
5. **Repo migration** — Deferred. Develop and publish `@dawcore/*` packages from the existing `naomiaro/waveform-playlist` repo for now. Transfer to `dawcore` org later if desired — GitHub's transfer ownership preserves stars, watchers, forks, issues, PRs, and redirects old URLs.
